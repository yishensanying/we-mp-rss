...
CREATE TABLE we_articles (
    ...
    is_favorite     NUMBER(10)          DEFAULT 0,
...
CREATE INDEX ix_we_feeds_status ON we_feeds (status);
...
-- 12. we_filter_rules HTML过滤规则表
CREATE TABLE we_filter_rules (
    id                NUMBER(10) GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    mp_id             VARCHAR2(255 CHAR)  NOT NULL,
    rule_name         VARCHAR2(255 CHAR)  NOT NULL,
    remove_ids        CLOB,
    remove_classes    CLOB,
    remove_selectors  CLOB,
    remove_attributes CLOB,
    remove_regex      CLOB,
    remove_normal_tag NUMBER(10)          DEFAULT 0,
    status            NUMBER(10)          DEFAULT 1,
    priority          NUMBER(10)          DEFAULT 0,
    created_at        TIMESTAMP,
    updated_at        TIMESTAMP,
    CONSTRAINT pk_we_filter_rules PRIMARY KEY (id)
);

CREATE INDEX ix_we_filter_rules_mp_id ON we_filter_rules (mp_id);
